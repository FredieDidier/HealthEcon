...
points to the supplement. `tab_pooled_did` / `tab_pooled_did_composition` are
still generated by 07 (legacy names) but are NO LONGER \input anywhere —
`tab_main_gradient` supersedes them. Table 1 (summary stats) lives in the
